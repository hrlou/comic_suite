...
use walkdir;
use std::os::windows::process::CommandExt; // Add this import at the top
...
impl SevenZipImageArchive {
    pub fn new(path: &Path) -> Result<Self, ArchiveError> {
        ...
        {
            let rel_path = entry.path().strip_prefix(temp_dir.path()).unwrap().to_string_lossy().to_string();
    ...
    fn read_file_by_name(&self, filename: &str) -> Result<Vec<u8>, ArchiveError> {
        ...
        file.read_to_end(&mut buffer).map_err(|_| ArchiveError::NoImages)?;
        log::info!("Successfully read {} bytes from {:?}", buffer.len(), extracted_path);
        Ok(buffer)
    }
}
    ...
    fn read_manifest_string(&self) -> Result<String, ArchiveError> {
        match self.read_file_by_name("manifest.toml") {
            ...
            Err(_) => {
                ...
                Err(ArchiveError::ManifestError("manifest.toml not found".into()))
            }
        }
    }
