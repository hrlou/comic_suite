...
use eframe::egui::{ColorImage, TextureFilter, TextureOptions, Vec2, ScrollArea, ProgressBar, Layout, Align};
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            ScrollArea::both().auto_shrink([false; 2]).show(ui, |ui| {
                if let Some(img) = &*self.image_cache.lock().unwrap() {
                    ...
                    // Display the cached texture by reference
                    if let Some((_, handle)) = &*cache {
                        ui.add(egui::Image::new((handle.id(), disp_size)).sense(egui::Sense::drag()));
                    }
                }
            });
